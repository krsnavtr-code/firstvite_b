...
export const createCategory = async (req, res) => {
  try {
    ...
    // Auto-update sitemap when category is created
    updateSitemapAsync();
...
export const updateCategory = async (req, res) => {
  try {
    ...
    // Auto-update sitemap when category is updated
    updateSitemapAsync();
...
export const deleteCategory = async (req, res) => {
  ...
  try {
    ...
    try {
      const booksCount = await Book.countDocuments({ category: id }).session(
        session,
      );

      if (booksCount > 0) {
        console.error(
          `[Category] Cannot delete - ${booksCount} books are associated with this category`,
        );
        await session.abortTransaction();
        session.endSession();
        return res.status(400).json({
    ...
    try {
      ...
      if (!deletedCategory) {
        console.error(
          `[Category] Failed to delete - no document was deleted for ID: ${id}`,
        );
        await session.abortTransaction();
        session.endSession();
        return res.status(500).json({
      ...
      // Auto-update sitemap when category is deleted
      updateSitemapAsync();
